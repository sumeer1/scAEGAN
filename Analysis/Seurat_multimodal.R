...
rnaseq.seurat <- rnaseq.seurat %>% 
  ...
  FindNeighbors(dims = 1:20, reduction = "rpca") %>%
  FindClusters(resolution = 0.6) %>%
  RunUMAP(dims = 1:20, reduction.name = 'rumap', reduction = 'rpca')

rnaseq.idents <- Idents(rnaseq.seurat)
colnames(rnaseq)    <- paste0('cell', seq(1, ncol(rnaseq)))
colnames(atac)    <- paste0('cell', seq(1, ncol(atac)))
...
rnaseq.seurat <- NormalizeData(rnaseq.seurat, normalization.method = 'CLR', margin = 2) %>% 
  ScaleData() %>% 
  RunPCA(reduction.name = 'apca') %>%
  FindNeighbors(dims = 1:20, reduction = "apca") %>%
  FindClusters(resolution = 0.6) %>%
  RunUMAP(dims = 1:20, reduction.name = 'aumap', reduction = 'apca')
...
rnaseq.seurat <- FindMultiModalNeighbors(
  ...
  dims.list = list(1:20, 1:20), modality.weight.name = c("RNA.weight", 'ATAC.weight'))

rnaseq.seurat <- FindClusters(rnaseq.seurat, graph.name = "wsnn", algorithm = 3, 
...
ggsave(file.path(outDir, paste0('RNASEQ_ATAC_umap603.pdf')), p.vae.grid,
...
ggsave(file.path(outDir, paste0( 'ingrated_vs_inputClass_heatmap603.pdf')), p.vae.grid,
